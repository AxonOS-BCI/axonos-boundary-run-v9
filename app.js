(function (root) {
  ...
  function boot() {
    ...
    function afterTickFx(w, h) {
      ...
      fx.prevObjs.forEach((o, id) => {
        ...
        if (o.wasHit || o.lastX < w * 0.2) {
          ...
          } else if (HAZARDS.includes(o.type)) {
            fx.streak++;
            AudioKit.dodge(fx.streak);
            if (fx.streak >= 3) toast("CLEAN \u00d7" + fx.streak, "#87ffb5");
          }
        }
    ...
    function updateHints() {
      const hints = new Set();
      const near = engine.objects.filter(o => o.lane === engine.lane && !o.hit && o.x < 90000 && o.x > 14000);
      for (const o of near) {
        if (o.type === "raw_leak" && engine.vaultKeys > 0 && engine.shieldT === 0) hints.add("seal");
        if ((o.type === "stale_consent" || o.type === "memory_phisher") && engine.auditCount === 0) hints.add("audit");
        if (o.type === "unauthorized_app" && engine.quarantines === 0) hints.add("quarantine");
      }
      if (engine.consent < 52) hints.add("revoke");
      if (engine.latency > 62) hints.add("throttle");
    ...
    function drawObject(o, w, h, t, alpha) {
      ...
      const x = ((o.x - o.speed * alpha) / 100) / LOGICAL_W * w;
    ...
    function start() {
      ...
      const sw = $("swipeHint");
      if ("ontouchstart" in root) { sw.classList.remove("hidden"); root.setTimeout(() => sw.classList.add("hidden"), 6200); }
      renderWeather();
    ...
    canvas.addEventListener("touchend", e => { e.preventDefault(); if (!touch || !engine || paused) return; const t = e.changedTouches[0]; const dx = t.clientX - touch.x, dy = t.clientY - touch.y; if (Math.abs(dx) < 14 && Math.abs(dy) < 14) { touch = null; return; } if (Math.abs(dx) > Math.abs(dy)) engine.action(dx < 0 ? "left" : "right"); else engine.action(dy < 0 ? "jump" : "duck"); touch = null; }, { passive: false });
