...
for phrase in ["Boundary Run v9", "v9.3.0", "Ari", "Kibo", "RUN GAME", "Replay Proof", "Dogecoin", "DMwHAhqVNWf7dyEznukxCufNS5rjuP5MTp", "Not a medical device", "No telemetry", "Content-Security-Policy"]:
    if phrase not in text:
        raise SystemExit(f"required phrase missing: {phrase}")
for bad in ["v7.9.812", "v8.0.1", "v8.8.4", "v7.3.0", "v9.0.0", "v9.1.0", "v9.2.0", "ABI v3", "unstick-ui", "serviceWorker.register", "gtag(", "XMLHttpRequest", "navigator.sendBeacon"]:
    if bad in text:
...
for phrase in ["crypto.subtle.digest(\"SHA-256\"", "async function hashProof", "async function verifyProof", "validateSeed", "validateContracts", "Stable Boundary", "COLLISION_LEFT_RATIO", "passive: false", "guardian_gate", "bumpCombo", "spawnSwarm"]:
...
print("Audit OK: v9.3.0 security, deterministic hardening, CI/CD and no stale markers")
